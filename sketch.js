...
const LIMIT_IMAGE_SIZE_BASED_SCREEN = false;
const SCREEN_BASED_IMAGE_LIMITATION_PERCENT = 70;
...
function setup() {
	...
	drag = select( "#draggable" )
		.drop( handleFileDropped )
		.dragOver( handleFileDraggedOver )
		.dragLeave( handleFileDraggedLeave );
...
function renderProcess() {
	if ( LIMIT_IMAGE_SIZE ) {
		resizeCanvas( min( img.width, MAX_CANVAS_WIDTH ) + SIZE_INFO_WIDTH, min( img.height, MAX_CANVAS_HEIGHT ) );
	} else if ( LIMIT_IMAGE_SIZE_BASED_SCREEN ) {
		resizeCanvas( min( img.width, window.innerWidth * ( SCREEN_BASED_IMAGE_LIMITATION_PERCENT / 100 ) ) + SIZE_INFO_WIDTH, min( img.height, window.innerHeight * ( SCREEN_BASED_IMAGE_LIMITATION_PERCENT / 100 ) ) );
	} else {
		resizeCanvas( img.width + SIZE_INFO_WIDTH, img.height )
	}
...
function draw() {
	...
	if ( !img ) {
		...
		textSize( 48 );
		textAlign( CENTER );
		text( "Drag a file here", width / 2, height / 2 )
		pop();
		return
	}
